...
func runSyncSample(authToken, pixelId string) {
	...
	resp, r, err := capiClient.SendEventSync(*event)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ConversionApi.SendEventSync``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	// response from `SendData`: Response
	fmt.Fprintf(os.Stdout, "Response from `ConversionApi.SendEventSync`: %v\n", resp)
}
